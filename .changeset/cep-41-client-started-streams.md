...
- Client isolation and lifecycle hardening from review: sessions are scoped by sender pubkey (frames and colliding starts from another client are dropped, never acknowledged or cross-readable), the request's reserved output writer no longer intercepts a client-started stream's ping/pong/abort frames (which previously killed responsive uploads on probe timeout and hid client aborts from the tool), per-token counters and registry slots are cleaned up even when an abort publish fails, the payload writer is terminated when the session dies, and the input-session cache is sender-scoped, capped and evicted when the request responds.

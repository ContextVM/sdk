...
- 23f8560: Payment hardening follow-ups to the transparent-payment-durability work (adversarial review findings, low-risk subset).

  - **Explicit gating fails closed at capacity:** `AuthorizationStore` no longer silently LRU-evicts live pending entries — an evicted live payment disarms the dedup and a retry would mint a second invoice (the same class the transparent middleware already guards against). At capacity it purges expired entries and then refuses; the middleware distinguishes refusal reasons via the pending TTL, so a **same-identity retry still answers `-32043` (keep polling)** even when the store is full, while a _new_ identity gets a terminal `-32000` "Payment capacity reached, retry later" before any invoice is minted. `grant()` purges expired grants before falling back to LRU eviction, so a live unconsumed grant survives capacity pressure.
  ...
  - **Open-stream writer reuse on duplicate delivery:** a duplicate delivery of the same request event re-entered the inbound path and overwrote the writer reservation, orphaning the writer bound to the already-forwarded request (its keepalive timers could later evict the client's session via probe timeout). The existing reservation is now reused, and `releaseUnusedWriter` never disposes a writer the running invocation has already started streaming to — a suppressed duplicate's drop-cleanup must not kill the live stream (the writer's own lifecycle reaps it).
  - `LruCache.entries()` JSDoc corrected: iteration is least- to most-recently-used (capacity-pressure scans rely on the real order).

- 47f514b: Transparent payment robustness (CEP-8): prevent double charges and lost paid results.
...
- 64a27dc: Mirror the request's gift-wrap kind on all server send paths in `GiftWrapMode.OPTIONAL`.
...
- b05fa0c: feat(gateway): add optional CEP-8 payment gating support
...
- 9a723e9: feat(payments): renegotiable `payment_interaction` (mid-session upsert + reset-on-initialize)
...
- b469912: feat: CEP-8 Explicit Payment Gating lifecycle
...
- 25eb39d: fix: use req() to avoid dedup deadlock after applesauce-relay 6.0.3
...
- 26fa051: fix(transport): resolve open-stream and progress token conflicts
...
- 37430bd: fix: forward progress notifications for oversized transfer timeout
...
- 4610f71: fix(transport): add client-side event deduplication and improve open-stream error handling
...
- 81840e7: refactor(transport): measure final published event size for oversized transfer decisions
